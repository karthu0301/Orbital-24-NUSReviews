    ...
    const fetchQuestionAndReplies = async () => {
      try {
        ...
        if (questionDoc.exists()) {
          ...
          const flagged = data.flagged || filter.isProfane(data.text);  
    ...
    if (user && questionId) {
      ...
      const updateLastViewed = () => {
        updateDoc(userRef, {
          ...
          alert('Please check that your username is updated in your profile and try again!');
        });
      };
    ...
    if (flagReason.trim() !== '') {
      try {
        ...
        if (isFlaggingQuestion) {
          ...
          flagData = {
            ...flagData,
            questionContent: questionData.text, 
            type: 'question',
            askedByUid: questionData.askedByUid
          };

          await updateDoc(questionRef, { flagged: true });
          incrementFlaggedContributions(questionData.askedByUid);
          ...
          flagData = {
            ...
            questionContent: questionData.text,
            type: 'reply',
            respondedByUid: replyData.answeredByUid
          };

          await updateDoc(replyRef, { flagged: true });
          incrementFlaggedContributions(replyData.answeredByUid);
        }
