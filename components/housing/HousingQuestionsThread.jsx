...
import './HousingQuestionsThread.css';
...
const HousingQuestionsThread = () => {
  ...
  const handleReplySubmit = async (e) => {
    ...
    if (newReply.trim() !== '') {
      let fileUrl = null;
      if (attachedFile) {
        const fileRef = storage.ref(`housingReplies/${attachedFile.name}`);
        await fileRef.put(attachedFile);
        fileUrl = await fileRef.getDownloadURL();
      }

      await db.collection('housingQuestions').doc(questionId).collection('housingReplies').add({
        text: newReply,
        fileUrl,
      ...
      const repliesSnapshot = await db.collection('housingQuestions').doc(questionId).collection('housingReplies').orderBy('timestamp').get();
      setReplies(repliesSnapshot.docs.map(doc => ({ questionId: doc.id, ...doc.data() })));
    }
  };
