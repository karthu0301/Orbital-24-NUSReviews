...
import './FoodQuestionsThread.css';
...
const FoodQuestionsThread = () => {
  ...
  useEffect(() => { 
    const fetchQuestionAndReplies = async () => {
      const questionRef = doc(db, 'foodQuestions', questionId);
      ...
      const repliesRef = collection(db, 'foodQuestions', questionId, 'foodReplies');
  ...
  const handleReplySubmit = async (e) => {
    ...
    if (newReply.trim() !== '') {
      let fileUrl = null;
      if (attachedFile) {
        const fileRef = storage.ref(`foodReplies/${attachedFile.name}`);
        await fileRef.put(attachedFile);
        fileUrl = await fileRef.getDownloadURL();
      }

      await db.collection('foodQuestions').doc(questionId).collection('foodReplies').add({
        text: newReply,
        fileUrl,
      ...
      const repliesSnapshot = await db.collection('foodQuestions').doc(questionId).collection('foodReplies').orderBy('timestamp').get();
      setReplies(repliesSnapshot.docs.map(doc => ({ questionId: doc.id, ...doc.data() })));
    }
  };
...
export default FoodQuestionsThread;
